...
import {
  ...
  ISoftDeleteTodoItemCommand,
  SoftDeleteTodoItemCommand,
  ISoftDeleteTodoListCommand,
  SoftDeleteTodoListCommand,
} from '../web-api-client';
...
export class TodoComponent implements OnInit {
  ...
  deleteListConfirmed(): void {
    const command: ISoftDeleteTodoListCommand = {
      id: this.selectedList.id,
    };
    this.listsClient
      .softDelete(new SoftDeleteTodoListCommand(command))
      .subscribe(
        () => {
  ...
  deleteItem(item: TodoItemDto, countDown?: boolean) {
    ...
    if (item.id === 0) {
      ...
      const command: ISoftDeleteTodoItemCommand = {
        id: item.id,
      };
      this.itemsClient
        .softDelete(new SoftDeleteTodoItemCommand(command))
        .subscribe(
          () =>
            (this.selectedList.items = this.selectedList.items.filter(
              (t) => t.id !== item.id
