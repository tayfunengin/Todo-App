...
import {
  ...
  TagDto,
  UpdateTodoItemDetailWithTagCommand,
  ITagDto,
  IUpdateTodoItemDetailWithTagCommand,
  TagsClient,
} from '../web-api-client';
...
export class TodoComponent implements OnInit {
  ...
  onSubmit() {
    ...
    const searchText = this.itemsFilterForm.get('searchText')?.value;
    const tag = this.itemsFilterForm.get('tag')?.value;

    this.itemsClient.getTodoItemsWithFilter(listId, searchText, tag).subscribe(
      (result) => {
        debugger;
  ...
  addTag(todoItemId?: number) {
    debugger;
    const newTag: ITagDto = {
      id: 0,
      todoItemId: this.itemDetailsFormGroup.get('id').value,
      name: this.newTagName,
    };
    this.selectedItemTags.push(new TagDto(newTag));
    this.newTagOpen = false;
    this.newTagName = '';
  }

  removeTag(id: number) {
    const index = this.selectedItemTags.findIndex((x) => x.id == id);
    if (index > -1) this.selectedItemTags.splice(index, 1);
  }
  ...
  updateItemDetailsWithTags(): void {
    debugger;
    const dto: IUpdateTodoItemDetailWithTagCommand = {
      ...this.itemDetailsFormGroup.value,
      tags: this.selectedItemTags,
    };
    const item = new UpdateTodoItemDetailWithTagCommand(dto);

    this.itemsClient
      .updateItemDetailsWithTags(this.selectedItem.id, item)
      .subscribe(
        () => {
          if (this.selectedItem.listId !== item.listId) {
            this.selectedList.items = this.selectedList.items.filter(
